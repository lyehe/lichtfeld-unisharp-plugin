...
from unisharp.utils.gaussians import gaussian_attribute_stats, sanitize_gaussians_for_export, save_ply  # noqa: E402
from unisharp.utils.gsplat import GSplatRenderer  # noqa: E402
from unisharp.utils.inference_renderer import CubemapPanoramaRenderer  # noqa: E402
...
CAMERA_NAME_ALIASES = {"pinhole": "perspective", "erp": "panorama"}
PANORAMA_CAMERA_NAMES = {"panorama", "erp", "spherical"}
PERSPECTIVE_CAMERA_NAMES = {"perspective", "pinhole"}
FISHEYE_CAMERA_NAMES = {"fisheye", "fisheye624", "opencv_fisheye"}
NON_PANORAMA_CAMERA_NAMES = PERSPECTIVE_CAMERA_NAMES | FISHEYE_CAMERA_NAMES
...
def _normalized_camera_override(value: Any) -> CameraKind | None:
    camera = str(value or "auto").strip().lower()
    if camera == "auto":
        return None
    return CAMERA_NAME_ALIASES.get(camera, camera)  # type: ignore[return-value]


def _model_rays(model_output: dict[str, Any]) -> torch.Tensor | None:
    for key in ("geometry_rays", "unik3d_gt_rays", "unik3d_rays"):
...
def _positive_distance_cap_m(value: float) -> float | None:
    value = float(value)
...
def _should_load_panorama_native(
    *,
    ...
    forced = _normalized_camera_override(args.camera)
    if forced == "panorama":
        return True
    if forced in {"perspective", "fisheye"}:
        return False
    json_camera_name = _camera_name_from_json(camera_json_entry)
    if json_camera_name in PANORAMA_CAMERA_NAMES:
        return True
    if json_camera_name in NON_PANORAMA_CAMERA_NAMES:
        return False
...
def _initial_max_long_edge(
    *,
    ...
    if _should_load_panorama_native(image_path=image_path, args=args, camera_json_entry=camera_json_entry):
        return PANORAMA_MAX_LONG_EDGE
    return PERSPECTIVE_MAX_LONG_EDGE
...
def _render_panorama_frame_and_faces(
    panorama_renderer: CubemapPanoramaRenderer,
    gaussians: Any,
    *,
    extr_w2c: torch.Tensor,
    equ_h: int,
    ...
    cube_color, _, cube_alpha = panorama_renderer.render_cubemap(gaussians, extr_w2c, face_w=int(face_w))
    erp_color = panorama_renderer.cube_to_erp(cube_color, equ_h=int(equ_h), equ_w=int(equ_w), face_w=int(face_w))
    erp_alpha = panorama_renderer.cube_to_erp(cube_alpha, equ_h=int(equ_h), equ_w=int(equ_w), face_w=int(face_w))
...
def _run_model_pinhole(
    ...
    *,
    ...
    return model(
        ...
        depth_gt=None,
        distance_init_cap_m=_positive_distance_cap_m(distance_init_cap_m),
        return_aux=True,
    )
...
def _process_one(
    *,
    ...
    for reload_attempt in range(2):
        ...
        forced_camera_name = _normalized_camera_override(args.camera)
        ...
        if explicit_intrinsics is not None:
            camera_kind: CameraKind = "panorama" if json_camera_name in PANORAMA_CAMERA_NAMES else "perspective"
            ...
            forced_camera_name is None and (json_camera_name in PANORAMA_CAMERA_NAMES or aspect_camera_name == "panorama")
        ):
            ...
            elif json_camera_name in FISHEYE_CAMERA_NAMES:
                camera_kind = "fisheye"
            elif json_camera_name in PERSPECTIVE_CAMERA_NAMES:
                camera_kind = "perspective"
            else:
    ...
    export_scale_multiplier = float(getattr(args, "splat_scale", 1.0))
    gaussians_world = sanitize_gaussians_for_export(
        transform_gaussians_to_world(out["gaussians"], src_w2c),
        scale_multiplier=export_scale_multiplier,
    )
    gaussian_stats = gaussian_attribute_stats(gaussians_world)
    scale_stats = gaussian_stats["scale"]
    opacity_stats = gaussian_stats["opacity"]
    LOGGER.info(
        "Gaussian export | n=%d scale_median=%.6g scale_p10=%.6g scale_p90=%.6g opacity_median=%.4f scale_x=%.3f",
        int(gaussian_stats["count"]),
        float(scale_stats["median"] or 0.0),
        float(scale_stats["p10"] or 0.0),
        float(scale_stats["p90"] or 0.0),
        float(opacity_stats["median"] or 0.0),
    ...
    metadata = {
        ...
        "panorama_renderer": "unisharp.utils.inference_renderer.CubemapPanoramaRenderer",
        "low_pass_filter_eps": float(args.low_pass_filter_eps),
        "splat_scale_multiplier": float(export_scale_multiplier),
        "gaussian_attribute_stats": gaussian_stats,
    ...
    if bool(getattr(args, "return_gaussians", False)):
        result["gaussians"] = gaussians_world
    return result
...
def load_model(checkpoint_path: Path, device: torch.device) -> tuple[UnisharpFeatureModel, int]:
    return _load_model(checkpoint_path, device)


def load_camera_json(path: Path | None) -> Any:
    return _load_camera_json(path)


def slug_from_path(image_path: Path) -> str:
    return _slug_from_path(image_path)
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--splat-scale", type=float, default=1.25, help="Multiplier applied to exported Gaussian scales.")
    return p


def main() -> None:
    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
    configure_torchhub_cache()
    args = _build_argparser().parse_args()
    args._camera_json_data = load_camera_json(args.camera_json)
    device = torch.device(str(args.device))
    model, step = load_model(Path(args.checkpoint), device=device)
    renderer = GSplatRenderer(
        color_space="sRGB",
        background_color="black",
        low_pass_filter_eps=float(args.low_pass_filter_eps),
    ).to(device)
    panorama_renderer = CubemapPanoramaRenderer(renderer=renderer)
