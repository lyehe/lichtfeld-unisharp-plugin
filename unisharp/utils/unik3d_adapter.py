    ...
    *,
    ...
    if camera_params is not None:
        ...
        if cam_model != "fisheye624":
            raise ValueError(
                f"Unsupported camera_model={camera_model!r}; fisheye inference expects OPENCV_FISHEYE/Fisheye624."
            )
...
def _local_unik3d_root() -> Path:
    repo_root = Path(__file__).resolve()
    return repo_root.parents[2] / "UniK3D"


def _patch_local_unik3d_imports(unik3d_root: Path) -> None:
    try:
        from core.unik3d_patch import patch_unik3d_for_inference

        patch_unik3d_for_inference(unik3d_root)
    except Exception:
        pass


def _try_import_unik3d() -> Any:
    unik3d_root = _local_unik3d_root()
    if unik3d_root.exists():
        _patch_local_unik3d_imports(unik3d_root)
        root_str = str(unik3d_root)
...
def _load_local_unik3d_model_for_inference(
    backbone: str,
    *,
    pretrained: bool,
) -> torch.nn.Module:
    ...
    config_path = _local_unik3d_root() / "configs" / "eval" / f"{backbone}.json"
    if not config_path.is_file():
        raise FileNotFoundError(f"UniK3D eval config not found: {config_path}")
    config = json.loads(config_path.read_text(encoding="utf-8"))

    model = _with_unik3d_losses_skipped(UniK3D, lambda: UniK3D(config))
...
def _with_unik3d_losses_skipped(unik3d_cls: Any, factory: Any) -> torch.nn.Module:
    original_build_losses = unik3d_cls.build_losses

    def _skip_losses(self, _config):  # type: ignore[no-untyped-def]
        self.losses = {}

    try:
        unik3d_cls.build_losses = _skip_losses
        return factory()
    finally:
        unik3d_cls.build_losses = original_build_losses
...
def load_unik3d_model(
    ...
    try:
        model = _load_local_unik3d_model_for_inference(backbone, pretrained=pretrained)
        ...
        model = _with_unik3d_losses_skipped(
            UniK3D,
            lambda: UniK3D.from_pretrained(f"lpiccinelli/unik3d-{backbone}"),
        )
